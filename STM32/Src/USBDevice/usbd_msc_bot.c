...
void MSC_BOT_Init(USBD_HandleTypeDef *pdev) {
	...
	((USBD_StorageTypeDef *)pdev->pUserDataSTORAGE)->Init(0U);

	(void)USBD_LL_FlushEP(pdev, MSC_EPOUT_ADDR);
	(void)USBD_LL_FlushEP(pdev, MSC_EPIN_ADDR);

	/* Prapare EP to Receive First BOT Cmd */
	(void)USBD_LL_PrepareReceive(pdev, MSC_EPOUT_ADDR, (uint8_t *)&hmsc->cbw, USBD_BOT_CBW_LENGTH);
}
...
static void MSC_BOT_CBW_Decode(USBD_HandleTypeDef *pdev) {
	...
	if ((USBD_LL_GetRxDataSize(pdev, MSC_EPOUT_ADDR) != USBD_BOT_CBW_LENGTH) || (hmsc->cbw.dSignature != USBD_BOT_CBW_SIGNATURE) || (hmsc->cbw.bLUN > 1U) || (hmsc->cbw.bCBLength < 1U) ||
		...
		else if ((hmsc->bot_state != USBD_BOT_DATA_IN) && (hmsc->bot_state != USBD_BOT_DATA_OUT) && (hmsc->bot_state != USBD_BOT_LAST_DATA_IN)) {
...
void MSC_BOT_SendCSW(USBD_HandleTypeDef *pdev, uint8_t CSW_Status) {
	...
	(void)USBD_LL_Transmit(pdev, MSC_EPIN_ADDR, (uint8_t *)&hmsc->csw, USBD_BOT_CSW_LENGTH);
