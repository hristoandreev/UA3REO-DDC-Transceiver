...
void VOCODER_Process(void) {
	...
	if (!SD_workbuffer_current) {
        adpcm_encode_block(ADPCM_cnxt, (uint8_t *) &SD_workbuffer_A[SD_RecordBufferIndex], (size_t *) &outbuff_size,
                           VOCODER_Buffer, SIZE_ADPCM_BLOCK);
    } else {
        adpcm_encode_block(ADPCM_cnxt, (uint8_t *) &SD_workbuffer_B[SD_RecordBufferIndex], (size_t *) &outbuff_size,
                           VOCODER_Buffer, SIZE_ADPCM_BLOCK);
    }
