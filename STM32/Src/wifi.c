...
bool WIFI_getDXCluster_background(void) {
	...
	char url[64];
	char *url1 = "/trx_services/cluster2.php?background&band=";
...
void WIFI_checkFWUpdates(void) {
	...
	char url[128];
	sprintf(url, "/wolf/trx_services/check_fw_updates.php?dev=0&stm32=%s&fpga=%d.%d.%d&lcd=%s", version_string, FPGA_FW_Version[2], FPGA_FW_Version[1], FPGA_FW_Version[0], ota_config_lcd);
